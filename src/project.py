...
class Project(ProjectCaptcha, Script):
    ...
    def get_info(self, api, endpoint="project/{0}/info"):
        endpoint=endpoint.format(self.project_obj['result']["id"])
    ...
    def make_new_thing(self, thing_name, description, api, endpoint="thing/new", verb="post"):
        data={"name":thing_name, "description":description, 'projectId': self.project_obj['result']["id"]}
    ...
    #     endpoint=endpoint.format(self.project_obj['result']["id"])
    #     data={"projectId": self.project_obj['result']["id"], "pageNumber":page_number, "pageSize":page_size}
